...
**TCP_STREAM (bulk), Mbit/s**: roughly neutral on bare-metal loopback (kernel
TSO already amortizes per-packet cost), but a large win container-to-container,
...
| `--busy-poll-us N` | set the ring busy-poll budget (writes `net.core.busy_read`; see *Busy polling*) |
| `--ring-kbytes N` | per-direction ring size in KiB (writes the `ring_kbytes` module parameter; see *Ring size*) |
